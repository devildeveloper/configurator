...
Configurator.prototype._getIterator = function(key, create) {
    ...
    var undefined, iterator = this.root;

    for( var i = 0, ii = key.length - 1; i < ii; i++ ) {
        var cur = key[i];

        if( iterator[cur] === undefined ) {
            if( create ) {
                iterator[cur] = {};
...
Configurator.prototype.load = function(arg1, arg2, cb) {
    ...
    var undefined, key, file, store, self = this;
 ...
 * @throws {Error} No store type found matching the file type
 */
Configurator.prototype.save =
Configurator.prototype.store = function(arg1, arg2, cb) {
    'use strict';

    var undefined, key, file, store, value, self = this;
...
Configurator.prototype.merge = function(key, value, replaceKey) {
    ...
    var keys = this._getKey( key ), lastKey = _.last( keys );

    var iterator = this._getIterator( keys, true );

    var newValue, lastValue = iterator[lastKey];
    ...
    return iterator[lastKey] = _.merge( newValue, value );
};
...
Configurator.prototype.get = function(key) {
    ...
    var undefined, keys = this._getKey( key );
...
Configurator.prototype.clear = function(key) {
    ...
    var undefined, keys = this._getKey( key ), lastKey = _.last( keys );

    var iterator = this._getIterator( keys, false );

    if( iterator !== undefined ) {
        iterator[lastKey] = undefined;

        delete iterator[lastKey];
    }
};
